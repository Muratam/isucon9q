...
Create index hoge2 on items(status, category_id, created_at);
Create index seller_id_status_created_at on items(seller_id, status, created_at);
Create index hoge3 on items(buyer_id);
create index hoge4 on items(status, timedateid);
